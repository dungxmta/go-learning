// source: github.com/qntfy/kazaam@v3.4.8/transform/util.go
//
// -> just copy it here to public internal function
...
func GetJSONRaw(data []byte, path string, pathRequired bool, keySeparator string) ([]byte, error) {
		...
		if arrayRefs != nil && len(arrayRefs) > 0 {
			...
			if arrayKeyStr == "*" {
				...
				_, err := jsonparser.ArrayEach(data, func(value []byte, dataType jsonparser.ValueType, offset int, err error) {
					results = append(results, transform.HandleUnquotedStrings(value, dataType))
				}, beforePath...)
				if err == jsonparser.KeyPathNotFoundError {
					if pathRequired {
						return nil, transform.NonExistentPath
					}
				} else if err != nil {
					return nil, err
				}
	...
	if dataType == jsonparser.String {
		...
		result = transform.HandleUnquotedStrings(result, dataType)
	}
	if len(result) == 0 {
		result = []byte("null")
	}
...
func delJSONRaw(data []byte, path string, pathRequired bool, keySeparator string) ([]byte, error) {
	...
	for element, k := range splitPath {
		...
		if arrayRefs != nil && len(arrayRefs) > 0 {
			...
			if arrayKeyStr == "*" {
				return nil, transform.SpecError("Array wildcard not supported for this operation.")
			}
...
func validateArrayKeyString(arrayKeyStr string) error {
	if arrayKeyStr != "*" && arrayKeyStr != "+" && arrayKeyStr != "-" {
		...
		if val < 0 || err != nil {
			return transform.ParseError(fmt.Sprintf("Warn: Unable to coerce index to integer: %v", arrayKeyStr))
		}
	}
	return nil
}
